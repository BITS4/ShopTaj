  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 py-8 space-y-6">
      ...
      <div className="border rounded-xl overflow-hidden">
        <h2 className="font-semibold px-4 py-3 border-b bg-muted/30">Items</h2>
        {order.items.map((item: any) => (
          <div key={item.id} className="flex gap-4 px-4 py-3 border-b last:border-0">
            ...
            <div className="flex-1 text-sm">
              ...
              <p className="mt-1">Qty: {item.quantity} · {formatPrice(item.priceAtPurchase)} each</p>
            </div>
      ...
      {order.shippingAddress && (
        <div className="border rounded-xl p-4">
          ...
          <p className="text-sm text-muted-foreground">{order.shippingAddress.street}, {order.shippingAddress.city}, {order.shippingAddress.country} {order.shippingAddress.zip}</p>
        </div>
      )}
