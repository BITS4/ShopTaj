...
export default function SellerProductsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<SellerProductsResponse>('/seller/products')
      return data
    },
  })
              ...
              <CardContent className="py-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {p.category?.name} · {Number(p.price).toFixed(2)} сом · Stock: {p.stock}
                  </p>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${p.isActive ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground'}`}
                  >
